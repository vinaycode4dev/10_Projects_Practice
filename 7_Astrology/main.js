...
form.addEventListener('submit',(e)=>{
    ...
    const userDataDisplay=`Hi, ${name} ${surname}, Your Zodiac Sign is '${zodiacSigns[month-1]}'. ${compliments[day-1]} ${victimCardCompliments[year%20]} ${recommendations[(day*month)%30]} ${predictions[(name.length*surname.length)%20]}`;
    
    //diplaying
    const display=document.getElementById("result");
    display.textContent=userDataDisplay; 
